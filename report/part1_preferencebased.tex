% ============================================================
% PHẦN 10.6 & 10.7: PREFERENCE-BASED SETTING & OTHER RANKING CRITERIA
...
Để đo mức độ sai lệch giữa thứ hạng dự đoán $\sigma$ và thứ hạng mục tiêu $\sigma^*$ trên tập $X$ có $n \geq 1$ phần tử, sách giáo khoa định nghĩa:
...
\begin{theorem}[Chặn mất mát và regret cho Sort-by-Degree, Định lý 10.4]
...
Chứng minh đi theo hai bước. Đặt $p_{uv} = \mathbb{E}_{\sigma^*|X}[\mathbf{1}_{\sigma^*(v) < \sigma^*(u)}]$ là xác suất $v$ được xếp trước $u$ trong thứ hạng mục tiêu.

\textbf{Bước 1: Phân tích mất mát của SBD.} Với cặp $(u,v)$, thuật toán SBD xếp $u$ trước $v$ khi $\mathrm{deg}_h(u) \geq \mathrm{deg}_h(v)$. Mất mát xảy ra khi SBD xếp $u$ trước $v$ (do $\mathrm{deg}_h(u) > \mathrm{deg}_h(v)$) nhưng thực tế $v$ nên xếp trước $u$ (tức $p_{uv}$ lớn).

\textbf{Bước 2: Liên hệ với mất mát của $h$.} Bằng cách khai thác điều kiện PIIA, người ta chỉ ra được rằng mỗi sự kiện ``SBD xếp sai một cặp'' tương ứng với việc hàm ưu tiên $h$ mắc ít nhất một lỗi trên cặp đó (theo một nghĩa trung bình). Phân tích cẩn thận dẫn đến hệ số 2 trong chặn.

Hệ số 2 xuất hiện vì mỗi lỗi của $h$ trên một cặp có thể gây ra tối đa 2 lỗi xếp hạng sau khi tổng hợp qua sort-by-degree.
\end{proof}
...
% -------- Lower bound --------
\subsubsection{Chặn Dưới Cho Thuật Toán Xác Định (Định Lý 10.5)}
...
\begin{theorem}[Chặn dưới cho thuật toán xác định -- Định lý 10.5]
...
\begin{morong}
\textbf{Tại sao chu trình là "kẻ thù" của thuật toán xác định?}

Bằng chứng trên cho thấy: khi hàm ưu tiên $h$ cảm sinh chu trình $u \succ_h w \succ_h v \succ_h u$, thuật toán xác định không thể làm hài lòng đồng thời tất cả các cặp. Nó buộc phải \emph{cam kết} một thứ tự cụ thể, và đối phương (adversary) luôn có thể chọn $\sigma^*$ để phạt nặng lựa chọn đó. Ngẫu nhiên hóa (randomization) cho phép thoát khỏi bẫy này bằng cách phân tán xác suất lỗi đều hơn trên các cặp.
\end{morong}
...
\begin{tikzpicture}[
    ...
    \node[box, fill=yellow!20] (pivot) at (0,0) {Pivot $u$ \\ (chọn ngẫu nhiên)};
    \node[box, fill=blue!10] (left) at (-3.5,-1.5) {Bên trái \\ $\{v: h(v,u) > \text{rand}\}$};
    \node[box, fill=red!10] (right) at (3.5,-1.5) {Bên phải \\ $\{v: h(u,v) > \text{rand}\}$};
    \draw[arr] (pivot.south west) -- (left.north east) node[midway, left, font=\scriptsize, sloped] {xác suất $h(v,u)$};
    \draw[arr] (pivot.south east) -- (right.north west) node[midway, right, font=\scriptsize, sloped] {xác suất $h(u,v)$};
    \node[font=\scriptsize, gray] at (0,-2.2) {Kết quả: [đệ quy trái] $+$ [$u$] $+$ [đệ quy phải]};
\end{tikzpicture}
\end{center}

\begin{theorem}[Chặn hiệu năng cho QuickSort ngẫu nhiên -- Định lý 10.4 mở rộng]
...
\begin{morong}
\textbf{Tại sao ngẫu nhiên hóa giúp loại bỏ hệ số 2?}

Trực giác: thuật toán xác định phải \emph{cam kết} một hướng cho mỗi cặp đảo vị (ví dụ: luôn xếp $u$ trước $v$ nếu $\mathrm{deg}(u) > \mathrm{deg}(v)$). Trong trường hợp chu trình, quyết định này có thể gây lỗi hệ thống.

Thuật toán ngẫu nhiên \emph{phân tán xác suất} theo đúng mức $h(u,v)$ cho mỗi cặp. Điều này đảm bảo rằng về mặt kỳ vọng, xác suất xếp sai một cặp bằng chính xác mức ``tin cậy sai'' của $h$ cho cặp đó -- không có thêm lỗi hệ thống nào.

Nói cách khác, QuickSort ngẫu nhiên là một \emph{unbiased estimator} của hàm ưu tiên $h$, trong khi sort-by-degree là một \emph{biased estimator} không tránh khỏi hệ số 2.
\end{morong}
...
\begin{tikzpicture}[scale=0.9]
    % Draw ranking positions
    \foreach \i/\rel/\col in {1/3/blue!60, 2/2/blue!40, 3/0/gray!30, 4/1/blue!20, 5/0/gray!30} {
        \draw[fill=\col, draw=black] (\i*2-2, 0) rectangle (\i*2-0.2, \rel*0.5+0.1);
        \node[font=\small] at (\i*2-1.1, -0.3) {Vị trí $\i$};
        \node[font=\small, white] at (\i*2-1.1, \rel*0.25+0.05) {\ifnum\rel>0 $r=\rel$\fi};
    }
    % Discount line
    \draw[red, thick, dashed] plot[smooth] coordinates {(0.5,1.0) (1.9,0.63) (3.9,0.5) (5.9,0.43) (7.9,0.39)};
    \node[red, font=\small] at (8.5, 0.39) {$c_i$};
    % Axes
    \draw[->] (-0.2,0) -- (9.5,0) node[right, font=\small] {Vị trí $i$};
    \draw[->] (0,-0.5) -- (0,1.8) node[above, font=\small] {Điểm liên quan $r_i$ (trọng số $c_i$)};
\end{tikzpicture}
\caption{Minh họa DCG: cột màu xanh là điểm liên quan $r_i$ tại mỗi vị trí, đường đỏ là hệ số chiết khấu $c_i$ giảm dần. DCG là tổng tích $c_i \cdot r_i$ trên tất cả vị trí.}
...
\paragraph{Liên hệ với Score-based Setting (Mục 10.1).}
...
\paragraph{Liên hệ với RankBoost (Mục 10.4).}
...
\paragraph{Liên hệ với Bipartite Ranking (Mục 10.5).}
...
\begin{enumerate}
    ...
    \item \textbf{Sort-by-degree:} Hệ số 2, độ phức tạp $\Omega(n^2)$. Định lý 10.5 chứng minh không thể cải thiện hệ số này với thuật toán xác định.
